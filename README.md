...
2. 입력창에 붙여넣고 **다운로드** 버튼 클릭 → 워터마크 없는 mp4 저장

- **AVC(h264) 우선 선택**: TikTok 은 h264(대개 540~720p)와 h265(대개 1080p)를 함께 제공하는데, 호환성과 속도를 위해 h264 를 우선 선택합니다 (변환 불필요 → 즉시 다운로드). h265 만 제공되는 영상은 서버가 자동으로 AVC 변환합니다.
...
### 일괄 다운로드에서 다수 항목이 "알 수 없는 서버 오류" 로 실패 (NAS)
→ HEVC 변환 작업이 몰려 리버스 프록시 타임아웃(기본 60초)을 초과했을 가능성. ① Synology 리버스 프록시 타임아웃을 600초로 연장([NAS_DEPLOYMENT.md](./NAS_DEPLOYMENT.md) 참고), ② 프론트/서버 코드가 최신인지 확인 (TikTok AVC 우선 선택 + 서버 동시 변환 제한이 적용된 버전).
