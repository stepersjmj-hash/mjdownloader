...
**고급 설정 탭 → 프록시 읽기 시간 제한(proxy_read_timeout)** 을 **600초** 로 설정.
HEVC → AVC 변환이나 대용량 다운로드는 서버가 첫 응답을 보내기까지 1분 이상 걸릴 수 있는데,
기본값(60초)이면 프록시가 백엔드 응답을 기다리다 연결을 끊어 브라우저에 "알 수 없는 서버 오류" 로 표시됩니다.
- 프록시 **연결**/(요청)**보내기** 시간 제한은 기본값 그대로 둬도 됨 (접속·요청 전송은 순식간)
- DSM 버전에 따라 이 옵션이 UI 에 없으면 `/etc/nginx/sites-enabled/server.ReverseProxy.conf` 의
  `proxy_read_timeout` 값을 직접 조정 — DSM 업데이트 시 초기화될 수 있음에 유의
